...
운영체제 과정의 가장 처음 과정은 프로세스(process)로 시작한다. 컴퓨터의 모든 것은 프로세스로 이루어졌다고 할 수 있을 정도로, 프로세스는 CS를 배우기 시작과 끝에 있는 아주 중요한 개념이다. 프로세스는 실행 중인 `프로그램` 이다. 프로그램은 하드 디스크(혹은 낸드 플래시)에 저장된 정적인 데이터 덩어리를 말하며, 이들에게 일련의 신호를 거쳐 
